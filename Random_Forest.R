...
env_vars <- read_csv("data/environmental_vars_checklists_md_jan.csv")

# zero-filled ebird data combined with environmental data
checklists_env <- read_csv("data/checklists_zf_md_deju_jan.csv") |> 
  inner_join(env_vars, by = "checklist_id")

# prediction grid
pred_grid <- read_csv("data/environmental_vars_prediction_grid_md.csv")
# raster template for the grid
r <- rast("data/prediction_grid_md.tif")
...
title <- "Predicted Jan. DEJU Encounter Rate (DNN)"
...
# # TEST
# # Read prediction CSV from Python
# predictions <- read_csv("junco_nn_predictions.csv")
# r_pred <- predictions |>
#   st_as_sf(coords = c("x", "y"), crs = crs) |>
#   select(in_range, encounter_rate) |>
#   rasterize(r, field = c("in_range", "encounter_rate"))
...
# 
# # Read predictions
# preds <- read_csv("dnn_predictions_for_r.csv")
# 
# # Run MCC-F1 evaluation
# mccf1_result <- mccf1(response = preds$obs, predictor = preds$pred)
# 
# # View best MCC and threshold
# summary(mccf1_result)
# 
# library(pROC)
# 
...
# library(precrec)
# 
# evalmod(scores = preds$pred, labels = preds$obs) |> autoplot()
